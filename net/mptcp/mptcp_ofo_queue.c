				 ...
				 const struct tcp_sock *tp)
{
	...
	if (seq == TCP_SKB_CB(here)->end_seq) {
		...
		if (!tcp_try_coalesce(meta_sk, here, skb, &fragstolen)) {
			__skb_queue_after(&meta_tp->mpcb->mptcp_ofo_queue, here, skb);
			return 1;
		} else {
			 ...
			 struct sk_buff_head *head, struct tcp_sock *tp)
{
	...
	if (seq == TCP_SKB_CB(skb1)->end_seq) {
		...
		if (!tcp_try_coalesce(meta_sk, skb1, skb, &fragstolen)) {
			__skb_queue_after(&meta_tp->mpcb->mptcp_ofo_queue, skb1, skb);
		} else {
			kfree_skb_partial(skb, fragstolen);
			skb = NULL;
		}
			      ...
			      struct sock *sk)
{
	...
	try_shortcut(tp->mptcp->shortcut_ofoqueue, skb,
		     &tcp_sk(meta_sk)->mpcb->mptcp_ofo_queue, tp);
}
...
bool mptcp_prune_ofo_queue(struct sock *sk)
{
	...
	if (is_meta_sk(sk)) {
		if (!skb_queue_empty(&tp->mpcb->mptcp_ofo_queue)) {
...
void mptcp_ofo_queue(struct sock *meta_sk)
{
	...
	while ((skb = skb_peek(&meta_tp->mpcb->mptcp_ofo_queue)) != NULL) {
		...
		if (!after(TCP_SKB_CB(skb)->end_seq, meta_tp->rcv_nxt)) {
			__skb_unlink(skb, &meta_tp->mpcb->mptcp_ofo_queue);
...
{
	struct sk_buff_head *head = &meta_tp->mpcb->mptcp_ofo_queue;
